...
import (
	...
	// Services & middleware
	iamService "github.com/gurkanfikretgunak/masterfabric-go/internal/domain/iam/service"
	...
	tenantRepo "github.com/gurkanfikretgunak/masterfabric-go/internal/domain/tenant/repository"
)
...
func New(deps Dependencies) *chi.Mux {
	...
	// Emotion & Focus Journal Endpoints
	if deps.JournalHandler != nil {
		r.Get("/", deps.JournalHandler.RootIndex)
		r.Get("/health", deps.JournalHandler.HealthCheck)
		r.Get("/version", deps.JournalHandler.GetVersion)
		r.Post("/feedback", deps.JournalHandler.SubmitFeedback)

		// Auth
		r.Post("/register", deps.JournalHandler.Register)
		r.Post("/login", deps.JournalHandler.Login)
		r.Post("/logout", deps.JournalHandler.Logout)
		r.Post("/refresh", deps.JournalHandler.RefreshToken)
		r.Get("/profile", deps.JournalHandler.GetProfile)
		r.Put("/profile", deps.JournalHandler.UpdateProfile)
		r.Put("/password", deps.JournalHandler.UpdatePassword)
		r.Delete("/delete", deps.JournalHandler.DeleteAccount)
